...
$(document).ready(function() {
  ...
  //Query the data
  function queryData(proffesion, item) {
    console.log("queryData");
    //Grab the right data variable
    var data = window[item];
    //Loop over the data to find the right info
    $.each(data, function(index, object){
      if(object.job.toLowerCase() == proffesion.toLowerCase()) {
        //Update all the fields
        $("#menstime").text(secondsToString(data[index].men))  ;
  ...
  function changeImage(chosenobject) {
    ...
    if (chosenobject == "nike") {
      $objectImage.attr('src','images/nike.png');
    } else if (chosenobject == "house") {
      $objectImage.attr('src','images/house.png');
...
function numberAppear() {
  ...
  function secondsToString(seconds) {
    ...
    return pad(numyears) + ":" + pad2(numdays) + ":" + pad(numhours) + ":" + pad(numminutes) + ":" + pad(numseconds);
  }
